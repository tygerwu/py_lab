...
class Int8Quantizer(BaseQuantizer):
    ...
    def init_quantize_config(self, operation: Operation) -> OperationQuantizationConfig:
        ...
        if operation.type in {'Conv', 'ConvTranspose', 'MatMul', 'Gemm', 'PPQBiasFusedMatMul'}:
            ...
            OQC.input_quantization_config[1].quant_min = - \
                pow(2, self.num_of_bits - 1)
            OQC.input_quantization_config[1].quant_max = pow(
                2, self.num_of_bits - 1) - 1
            ...
            raise TypeError(
                f'Op type {operation.type} is not supported by this quantizer.')
